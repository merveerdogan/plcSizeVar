...
jsPsych.plugins["displayCreator"] = (function () {
    ...
    plugin.trial = function (display_element, trial) {
        ...
        function sampleLocationShift() {
            ...
            const px = prevClothLocation[0], py = prevClothLocation[1];
        ...
        function move_disc() {
            ...
            if (time_elapsed < displayDuration) {
                ...
                for (let i = 0; i < selectedDotPosX.length; i++) {
                    ...
                    let dotPosX = currentClothLocation[0] + (orig[0] - currentClothLocation[0]) * currentSizeFactor;
                    let dotPosY = currentClothLocation[1] + (orig[1] - currentClothLocation[1]) * currentSizeFactor;
                ...
                if (idx !== lastDisplayedFrameIndex) {
                    ...
                    if (frameDetails.clothPos.length > 0) {
                        ...
                        let shiftDistance = Math.sqrt((actualClothCenterX - prevPos[0]) ** 2 + (actualClothCenterY - prevPos[1]) ** 2);
                        frameDetails.locationShiftDistancePx.push(round2(shiftDistance));
                        if (shiftDistance < trial.minLocationShift && trial.locationShiftEnabled) {
                            console.log('WARNING: Shift distance', round2(shiftDistance), 'is less than min', trial.minLocationShift);
                        }
                    } else if (trial.locationShiftEnabled) {
                        // Record the initial actual cloth center as the first position
        ...
        function end_trial() {
            ...
            // Console log location shift summary
            if (trial.locationShiftEnabled && locationShiftDistancePxStats.avg !== null) {
                console.log('Location Shift Summary:');
                console.log('  Min shift distance:', round2(locationShiftDistancePxStats.min), 'px');
                console.log('  Max shift distance:', round2(locationShiftDistancePxStats.max), 'px');
                console.log('  Avg shift distance:', round2(locationShiftDistancePxStats.avg), 'px');
            }
            ...
            var trial_data = {
                ...
                scaleFactorActualized: new Set(frameDetails.scaleFactorFrameToFrame), //set of all scale factors that occurred in the trial
