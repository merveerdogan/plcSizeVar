...
// Fullscreen monitor & warning overlay
function initFullscreenMonitor() {
    if (document.getElementById('fullscreen-warning-overlay')) return;
    ...
    msg.textContent = 'You left fullscreen. Please return to continue the experiment.';
    ...
    function recordFullscreenEvent(leftFullscreen) {
        try {
            var last = jsPsych && jsPsych.data ? (jsPsych.data.get().last(1).values()[0] || {}) : {};
            var duringTrialType = last.trial_type || null;
            var duringCategory = last.trial_category || null;
            var duringIndex = (typeof last.trial_index !== 'undefined') ? last.trial_index : null;
            if (jsPsych && jsPsych.data && jsPsych.data.write) {
                jsPsych.data.write({
                    event: leftFullscreen ? 'fullscreen_exit' : 'fullscreen_enter',
                    during_trial_type: duringTrialType,
                    during_trial_category: duringCategory,
                    during_trial_index: duringIndex,
    ...
    function onFsChange() {
        ...
        if (!inFs) {
            ...
            recordFullscreenEvent(true);
        } else {
            overlay.style.display = 'none';
            document.body.style.cursor = 'none';
            recordFullscreenEvent(false);
        }
    }
...
var debreif_qs = {
    ...
    on_finish: function (data) {
        ...
        saveData(participantID, test_data.csv(), save_folder_filtered);
